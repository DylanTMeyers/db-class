...
app.get('/search', async (req, res) => {
    ...
    try {
	    ...
	    const response = await pool.query(template, [searchTerm]);
	     ...
	     const campgrounds = response.rows.map((row) => {return {campground:`${row.name}`, location: `${row.location}`,maxlength: row.maxlength ,elevation: row.elevation,sites: row.sites,padtype: `${row.padtype}`}});
...
app.get('/fit', async (req, res) => {
	...
	const template = "SELECT * FROM campgrounds";
	
	const response = await pool.query(template);
		 
	for(var i = 0; i< response.rows.length; i++){
		
                 if(response.rows[i].maxlength >= searchMax) {
			                                     results.push({campground: `${response.rows[i].name}`, location: `${response.rows[i].location}`,maxLength: response.rows[i].maxlength});
							                                        }
	}
...
app.get('/elevation', async (req, res) => {
    ...
    let higher=req.query.altitude;
    let query = '';
	
    if (req.query.direction == 'lower'){
        query = "SELECT * FROM campgrounds WHERE "+ higher + "> elevation";
    } else {
        query =  "SELECT * FROM campgrounds WHERE " + higher + "< elevation";
    ...
    try {
	 ...
	 const response = await pool.query(query);
	    for(var i = 0; i< response.rows.length; i++){
	    results.push({campground: `${response.rows[i].name}`, elevation: `${response.rows[i].elevation}`,town: response.rows[i].location});
	    }
        res.json({campgrounds: results});
